...
The PTP HW Master receives the necessary information from the USER-APP fields of the AXI MCDMA for capturing timestamp and storing it in the memory for the Transmitted PTP packets. The address of the memory where the timestamp needs to be stored is provided by the USER-APP fields of AXI MCDMA Control stream.
...
The PTP HW Master in the zcu670 TRD supports 2-step PTP mode of operation.

**2-step Mode:**

When the ptp flag value is “10”, the PTP HW Master initiates a memory write operation of the timestamp received from Timer syncer IP. The timestamp value is written into the memory via a AXI memory mapped interface. The PTP HW Master sets a bit to 1 to indicate the completion of memory write to Software.


### Rx PTP PACKET DETECT and PREPEND LOGIC IP

* PTP Packet Detection IP parses all the received Packets.

* When a PTP packet is received, it parses the Header and sets a PTP Qualifier bit to '1' when a PTP packet is detected, that helps the PTP application in upper layer for further processing.
* PTP detect logic detects PTP packets considering the following PTP transport mechanism.
